...
class Replies extends BaseCommand {
  ...
  handle () {
    ...
    this.hears(/vo(u*) chor(a+).*$/i, matches => { 
      this.send(this.channel, "chora não lindx :heart:")
    })

    this.hears(/.+(duo\?)$/i, matches => { 
      this.send(this.channel, "DUO!")
    })

    this.hears(/^(\?{2,})$/i, matches => { 
      this.send(this.channel, `${Array(matches[1].length + 1).join('?')}`)
    })

    this.hears(/^eai nerds$/i, matches => { 
      this.send(this.channel, "Fala cachorra")
    })
  }
