...
let manager = new Database(path.join(process.cwd(), 'db/customcommands.json'))
...
class Tags extends BaseCommand {
  ...
  handle () {
    ...
    this.responds(/^tags (list|l|\?)$/i, matches => {
      let tags = manager.getAll()
      var arrKeys = Object.keys(tags).sort()
      while(arrKeys.length) {
        this.send(this.sender, [
          '```',
          arrKeys.splice(0, 100).join(", "),
          '```'
        ].join('\n'));
      }
